...
const Component = (props: ICarouselProps) => {
    ...
    const handleItemChange = (stateSet: (previous: number) => number) => {
        props.onItemChange && props.onItemChange(stateSet(current));
        setCurrent(stateSet);
    }
		...
		<div className={Styles.container(className)}>
			<div className='flex h-full w-full flex-row items-center justify-center gap-8'>
				<button
					...
					onClick={() =>
						handleItemChange((p) => (p == 0 ? data.length - 1 : p - 1))
					}
				>
					🠔
				</button>
				...
				<button
					className='transition-200 aspect-square h-12 w-12 cursor-none rounded-full border text-xl transition-all hover:bg-white hover:text-[#000]'
					onClick={() =>
						handleItemChange((p) => (p == data.length - 1 ? 0 : p + 1))
					}
				>
					🠖
				</button>
